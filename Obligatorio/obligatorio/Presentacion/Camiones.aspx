<%@ Page enableEventValidation="false" Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Camiones.aspx.cs" Inherits="obligatorio.Presentacion.Camiones" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <h1>Gestión de Camiones</h1>
    <form role="form">      <div class="form-group">          <br />        <label for="InputMarca">Marca</label>          <asp:TextBox runat="server" CssClass="form-control" ID="InputMarca" placeholder="Ingresa la marca"></asp:TextBox>          <asp:Label ID="lblMissingMat" runat="server"></asp:Label>
      </div>      <div class="form-group">        <label for="InputModelo">Modelo</label>          <asp:TextBox runat="server" CssClass="form-control" ID="InputModelo" placeholder="Ingresa el modelo"></asp:TextBox>      </div>      <div class="form-group">        <label for="InputMatricula">Matrícula</label>          <asp:TextBox runat="server" CssClass="form-control" ID="InputMatricula" placeholder="Ingresa la matrícula"></asp:TextBox>      </div>      <div class="form-group">        <label for="InputAno">Año</label>          <asp:TextBox runat="server" CssClass="form-control" ID="InputAno" placeholder="Ingresa el año"></asp:TextBox>          <asp:Label ID="lblIdNotNum" runat="server"></asp:Label>
      </div>        <asp:Button ID="btnAlta" runat="server" OnClick="btnAlta_Click" Text="Alta" />
        <asp:Button ID="btnMod" runat="server" Text="Modificar" OnClick="btnMod_Click" />
        <asp:Button ID="btnLimpiar" runat="server" OnClick="btnLimpiar_Click" Text="Limpiar" />
        <br />
        <asp:GridView ID="grdCamiones" runat="server" AutoGenerateColumns="False" AutoGenerateDeleteButton="True" AutoGenerateSelectButton="True" CellPadding="4" DataKeyNames="matriculaCamion" DataSourceID="Test1" EmptyDataText="No hay datos" GridLines="None" ShowHeaderWhenEmpty="True" ViewStateMode="Enabled" OnRowDeleting="grdCamiones_RowDeleting" OnSelectedIndexChanging="grdCamiones_SelectedIndexChanging">
            <Columns>
                <asp:BoundField DataField="matriculaCamion" HeaderText="Matricula" ReadOnly="True" SortExpression="matriculaCamion" />
                <asp:BoundField DataField="modelo" HeaderText="Modelo" SortExpression="modelo" />
                <asp:BoundField DataField="marca" HeaderText="Marca" SortExpression="marca" />
                <asp:BoundField DataField="ano" HeaderText="Año" SortExpression="ano" />
            </Columns>
        </asp:GridView>
        <asp:SqlDataSource ID="Test1" runat="server" ConnectionString="<%$ ConnectionStrings:ObligatorioProgramacion3ConnectionString %>" SelectCommand="SELECT [matriculaCamion], [modelo], [marca], [ano] FROM [Camion]"></asp:SqlDataSource>
        <br />
        <asp:Label ID="lblDataOutput" runat="server"></asp:Label>

    </form>

</asp:Content>
